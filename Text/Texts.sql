insert or replace into EnglishText (Tag, Text) values
  -- 文明领袖
  ("LOC_TRAIT_CIVILIZATION_AMAZON_DESCRIPTION_MONOPOLIES",          "Grant a Bandeirantes after establishing [ICON_CAPITAL] Capital city. All districts (except city centers) will not remove Rainforest. Rainforest tiles provide a standard adjacency bonus for specialty districts, and grant +1 Appeal to adjacent tiles. When you build Industry or Corporation on Rainforest tiles, immediately activate the corresponding bonus of all its usages."),
  -- 科技 市政
...
insert or replace into LocalizedText (Language, Tag, Text) values
  -- 文明领袖
  ("zh_Hans_CN",  "LOC_TRAIT_CIVILIZATION_AMAZON_DESCRIPTION_MONOPOLIES", "建立 [ICON_CAPITAL] 首都后获得一位探险家。除市中心外的区域不移除雨林。雨林单元格为专业化区域提供标准相邻加成，并为相邻的单元格提供+1魅力。建造在雨林上的行业或公司直接激活所有用途所对应的加成效果。"),
  -- 科技 市政
