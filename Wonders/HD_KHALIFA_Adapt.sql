...
select
	'REQUIRES_CITY_HAS_JNR_GRAND_HOTEL',	'REQUIREMENT_CITY_HAS_BUILDING'
where exists (select BuildingType from Buildings where BuildingType = 'BUILDING_JNR_GRAND_HOTEL');

insert or ignore into RequirementArguments
	(RequirementId,							Name,				Value)
select
	'REQUIRES_CITY_HAS_JNR_GRAND_HOTEL',	'BuildingType',		'BUILDING_JNR_GRAND_HOTEL'
where exists (select BuildingType from Buildings where BuildingType = 'BUILDING_JNR_GRAND_HOTEL');

insert or ignore into RequirementSets
	(RequirementSetId,						RequirementSetType)
select
	'HD_CITY_HAS_JNR_GRAND_HOTEL',			'REQUIREMENTSET_TEST_ALL'
where exists (select BuildingType from Buildings where BuildingType = 'BUILDING_JNR_GRAND_HOTEL');

insert or ignore into RequirementSetRequirements
	(RequirementSetId,						RequirementId)
select
	'HD_CITY_HAS_JNR_GRAND_HOTEL',			'REQUIRES_CITY_HAS_JNR_GRAND_HOTEL'
where exists (select BuildingType from Buildings where BuildingType = 'BUILDING_JNR_GRAND_HOTEL');

insert or replace into Modifiers
	(ModifierId,					ModifierType,								SubjectRequirementSetId)
select
	'KHALIFA_PRODUCT_TOURISM',		'MODIFIER_PLAYER_CITIES_ADJUST_TOURISM',	'HD_CITY_HAS_JNR_GRAND_HOTEL'
where exists (select BuildingType from Buildings where BuildingType = 'BUILDING_JNR_GRAND_HOTEL');
...
insert or ignore into CivilopediaPageExcludes (SectionId, PageId)
	values ('RESOURCES', 'RESOURCE_KHALIFA');
...
insert or ignore into Projects_XP2 (ProjectType, RequiredBuilding)
	values ('PROJECT_CREATE_PRODUCT_KHALIFA', 'BUILDING_BURJ_KHALIFA');
...
-- Products --
--------------
CREATE TEMPORARY TABLE "HD_KHALIFA_Products"(
    'Num'  TEXT
);
...
values
	('GREATWORK_PRODUCT_KHALIFA_1',		'YIELD_FOOD',		6),
	('GREATWORK_PRODUCT_KHALIFA_1',		'YIELD_FAITH',		6),
	('GREATWORK_PRODUCT_KHALIFA_2',		'YIELD_FOOD',		6),
	('GREATWORK_PRODUCT_KHALIFA_2',		'YIELD_PRODUCTION', 6),
	('GREATWORK_PRODUCT_KHALIFA_3',		'YIELD_GOLD',		24),
	('GREATWORK_PRODUCT_KHALIFA_4',		'YIELD_CULTURE',	6),
	('GREATWORK_PRODUCT_KHALIFA_4',		'YIELD_SCIENCE',	6),
	('GREATWORK_PRODUCT_KHALIFA_5',		'YIELD_SCIENCE',	12),
	('GREATWORK_PRODUCT_KHALIFA_6',		'YIELD_FOOD',		6),
	('GREATWORK_PRODUCT_KHALIFA_6',		'YIELD_GOLD',		12),
	('GREATWORK_PRODUCT_KHALIFA_7',		'YIELD_CULTURE',	12),
	('GREATWORK_PRODUCT_KHALIFA_8',		'YIELD_CULTURE',	6),
	('GREATWORK_PRODUCT_KHALIFA_8',		'YIELD_GOLD',		12);
...
values
    ...
    ('PRODUCT_KHALIFA_4_BONUS1',        'Amount',       0.3),
    ('PRODUCT_KHALIFA_4_BONUS2',        'YieldType',    'YIELD_SCIENCE'),
    ('PRODUCT_KHALIFA_4_BONUS2',        'Amount',       0.3),
