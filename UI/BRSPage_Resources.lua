...
-- 2023-05-10: Created
-- ===========================================================================
print("HD Override BRSPage_Resources.lua")
...
function ViewResourcesPage()
	...
	for eResourceType,kSingleResourceData in spairs(m_kResourceData, function(t,a,b)
		...
		-- 2021-05-12 Monopolies Mode
		local hasMonopoly = false;
		if bIsMonopolies and kSingleResourceData.IsLuxury then -- also check if there is an industry around it!
			...
			for row in GameInfo.HD_Monopoly_Resource_Categories() do
				if row.ResourceType == kResource.ResourceType then
			...
			local sEffectI:string = '';
			local sEffectC:string = '';
			...
			if #industryStr > 0 then
				...
				sEffectI = sEffectI .. Locale.Lookup('LOC_HD_INDUSTRY_EFFECT_TEXT', effectStr);
			end
			...
			if #corporationStr > 0 then
				...
				sEffectC = sEffectC .. Locale.Lookup('LOC_HD_CORPORATION_EFFECT_TEXT', effectStr);
			end
		...
		-- 2021-05-12 Monopolies Mode
		if bIsMonopolies and kSingleResourceData.IsLuxury and hasMonopoly and bMercantailismUnlocked then
			local pGameEconomic:table = Game.GetEconomicManager();
			local iControlled:number = pGameEconomic:GetNumControlledResources(localPlayerID, eResourceType);
